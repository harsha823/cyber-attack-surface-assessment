...
port_scanner.py
---------------
Runs an Nmap port scan against a target and returns structured results.

Usage:
    from scanner.port_scanner import PortScanner

    ps = PortScanner(target="192.168.56.101")
    results = ps.run_scan()
    ps.display_results(results)
"""
...
# ── Scan speed profiles ────────────────────────────────────────────────
SPEED_PROFILES = {
    "sneaky" : "-T2",   # slow — fewer packets, less likely to trigger IDS 
    "normal" : "-T3",   # default Nmap speed
    "fast"   : "-T4",   # faster — good for lab environments
}
...
class PortScanner:
    ...
    def run_scan(self) -> list:
        ...
        try:
            subprocess.run(
                command,
                check=True,          # raises exception if nmap exits with error
